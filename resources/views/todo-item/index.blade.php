<x-app-layout>
    ...
    <div class="py-12">
        <div class="text-center mb-5">
            <a href="{{ route('todoItems.create', ['todoList' => $todoList->id]) }}"
               class="mx-auto bg-green-500 text-white active:bg-green-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mb-4 ease-linear transition-all duration-15gr">Create</a>
        </div>
        <div class="max-w-md mx-auto sm:px-6 lg:px-8">
            <div
                class="mb-2 bg-white overflow-hidden shadow-sm sm:rounded-lg text-blue-600 ">{{ Session::get('userActionMessage') }}</div>
            ...
            <form class="mb-2" method="get" action="{{ route('todoItems.index', ['todoList' => $todoList->id]) }}">
                ...
                <div class="text-center mb-5">
                    <button type="submit"
                            class="bg-blue-500 text-white active:bg-green-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all duration-15">
                        Apply
                    </button>
                </div>
            </form>
            @foreach($todoItems as $todoItem)
                <div class="space-x-0 mb-2 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <p class="text-4xl text-center">{{ $todoItem->text }}</p>
                    <p class="text-2xl text-center">Status: {{ $todoItem->status }}</p>
                    @if($todoItem->status === \App\Models\TodoItem::STATUS_DONE)
                        <p class="text-2xl text-center">Done at: {{ $todoItem->status_set_at }}</p>
                    @endif
                    <a
                    ...
                    <form class="inline-block" method="post"
                          action=" {{ route('todoItems.destroy', ['todoList' => $todoList->id, 'todoItem' => $todoItem->id]) }} ">
                        @csrf
                        @method('DELETE')
                        <button
                    ...
                    @if($todoItem->status !== \App\Models\TodoItem::STATUS_DONE )
                        <form class="inline-block" method="post"
                              action=" {{ route('todoItems.updateStatus', ['todoList' => $todoList->id, 'todoItem' => $todoItem->id]) }} ">
                            @csrf
                            @method('PATCH')
