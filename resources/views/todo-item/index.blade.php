    ...
    <div class="py-12">
        ...
        <div class="max-w-md mx-auto sm:px-6 lg:px-8">
            ...
            <form class="mb-2" method="get" action="{{ route('todoItems.index', ['todoList' => $todoList->id]) }}">
                ...
                <div class="text-center mb-5">
                    <button type="submit" class="bg-blue-500 text-white active:bg-green-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all duration-15">Apply</button>
                </div>
            </form>
            @foreach($todoItems as $todoItem)
                <div class="space-x-0 mb-2 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <form class="inline-block" method="post" action=" {{ route('todoItems.updateStatus', ['todoList' => $todoList->id, 'todoItem' => $todoItem->id]) }} ">
                        ...
                        <button class="bg-green-500 text-white active:bg-green-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150">Set Done</button>
                    </form>
                    <form class="inline-block" method="post" action=" {{ route('todoItems.updateStatus', ['todoList' => $todoList->id, 'todoItem' => $todoItem->id]) }} ">
                        @csrf
                        @method('PATCH')
